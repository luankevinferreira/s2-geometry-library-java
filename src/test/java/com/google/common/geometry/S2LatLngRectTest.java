...
public strictfp class S2LatLngRectTest extends GeometryTestCase {
	...
	public void testIntervalOps(S2LatLngRect x, S2LatLngRect y, String expectedRelation, S2LatLngRect expectedUnion,
	...
	public void testBasic() {
		...
		// FromCenterSize()
		assertTrue(S2LatLngRect.fromCenterSize(S2LatLng.fromDegrees(80, 170), S2LatLng.fromDegrees(40, 60))
		...
		assertTrue(S2LatLngRect.fromCenterSize(S2LatLng.fromDegrees(-90, 180), S2LatLng.fromDegrees(20, 50))
		...
		assertEquals(S2LatLngRect.fromPointPair(S2LatLng.fromDegrees(-35, -140), S2LatLng.fromDegrees(15, 155)),
				rectFromDegrees(-35, 155, 15, -140));
		assertEquals(S2LatLngRect.fromPointPair(S2LatLng.fromDegrees(25, -70), S2LatLng.fromDegrees(-90, 80)),
		...
		for (int i = 0; i < 4; ++i) {
			...
			for (int k = 0; k < 4; ++k) {
				assertTrue(S2.simpleCCW(r.getVertex((k - 1) & 3).toPoint(), r.getVertex(k).toPoint(),
		...
		testIntervalOps(rectFromDegrees(-15, -160, -15, -150), rectFromDegrees(20, 145, 25, 155), "FFFF",
				rectFromDegrees(-15, 145, 25, -150), empty);
		...
		// Expanded()
		assertTrue(rectFromDegrees(70, 150, 80, 170).expanded(S2LatLng.fromDegrees(20, 30))
		...
		assertTrue(rectFromDegrees(-90, 170, 10, 20).expanded(S2LatLng.fromDegrees(30, 80))
				.approxEquals(rectFromDegrees(-90, -180, 40, 180)));

		// ConvolveWithCap()
		S2LatLngRect llr1 = new S2LatLngRect(S2LatLng.fromDegrees(0, 170), S2LatLng.fromDegrees(0, -170))
				.convolveWithCap(S1Angle.degrees(15));
		S2LatLngRect llr2 = new S2LatLngRect(S2LatLng.fromDegrees(-15, 155), S2LatLng.fromDegrees(15, -155));
		...
		assertTrue(new S2LatLngRect(S2LatLng.fromDegrees(-45, -45), S2LatLng.fromDegrees(45, 45)).getCapBound()
				.approxEquals(S2Cap.fromAxisHeight(new S2Point(1, 0, 0), 0.5)));
		// GetCapBound(), bounding cap at north pole is smaller:
		assertTrue(new S2LatLngRect(S2LatLng.fromDegrees(88, -80), S2LatLng.fromDegrees(89, 80)).getCapBound()
				.approxEquals(S2Cap.fromAxisAngle(new S2Point(0, 0, 1), S1Angle.degrees(2))));
		// GetCapBound(), longitude span > 180 degrees:
		assertTrue(new S2LatLngRect(S2LatLng.fromDegrees(-30, -150), S2LatLng.fromDegrees(-10, 50)).getCapBound()
		...
		testCellOps(rectFromDegrees(-41, 160, -40, -160), S2Cell.fromFacePosLevel(5, (byte) 0, 0), 1);
		{
			...
			testCellOps(rectFromDegrees(v0.lat().degrees() - 1e-8, v0.lng().degrees() - 1e-8,
		...
		testCellOps(rectFromDegrees(-37, -70, -36, -20), S2Cell.fromFacePosLevel(5, (byte) 0, 0), 2);
		{
			...
			testCellOps(rectFromDegrees(bound202.lo().lat().degrees() + 3, bound202.lo().lng().degrees() + 3,
	...
	public void testGetDistanceRandomPairs() {
		...
		for (int i = 0; i < 10000; ++i) {
			S2LatLngRect a = S2LatLngRect.fromPointPair(new S2LatLng(randomPoint()), new S2LatLng(randomPoint()));
			S2LatLngRect b = S2LatLngRect.fromPointPair(new S2LatLng(randomPoint()), new S2LatLng(randomPoint()));
	...
	private static S1Angle bruteForceDistance(S2LatLngRect a, S2LatLngRect b) {
		...
		S2LatLng pntA[] = { new S2LatLng(a.latLo(), a.lngLo()), new S2LatLng(a.latLo(), a.lngHi()),
				new S2LatLng(a.latHi(), a.lngHi()), new S2LatLng(a.latHi(), a.lngLo()) };
		S2LatLng pntB[] = { new S2LatLng(b.latLo(), b.lngLo()), new S2LatLng(b.latLo(), b.lngHi()),
		...
		S2Point lng_edge_a[][] = { { pntA[0].toPoint(), pntA[3].toPoint() }, { pntA[1].toPoint(), pntA[2].toPoint() } };
		S2Point lng_edge_b[][] = { { pntB[0].toPoint(), pntB[3].toPoint() }, { pntB[1].toPoint(), pntB[2].toPoint() } };
		...
		for (int i = 0; i < 4; ++i) {
			...
			for (int j = 0; j < 2; ++j) {
				...
				S1Angle aToLng = S2EdgeUtil.getDistance(currentA.toPoint(), lng_edge_b[j][0], lng_edge_b[j][1]);
				S1Angle bToLng = S2EdgeUtil.getDistance(currentB.toPoint(), lng_edge_a[j][0], lng_edge_a[j][1]);

				minDistance = S1Angle.min(minDistance,
						S1Angle.min(aToLat, S1Angle.min(bToLat, S1Angle.min(aToLng, bToLng))));
			}
		}
		return minDistance;
	}
	...
	private static S1Angle bruteForceRectPointDistance(S2LatLngRect a, S2LatLng b) {
		...
		S1Angle bToLoLng = S2EdgeUtil.getDistance(b.toPoint(), new S2LatLng(a.latLo(), a.lngLo()).toPoint(),
				new S2LatLng(a.latHi(), a.lngLo()).toPoint());
		S1Angle bToHiLng = S2EdgeUtil.getDistance(b.toPoint(), new S2LatLng(a.latLo(), a.lngHi()).toPoint(),
	...
	private static S2LatLngRect getEdgeBound(double x1, double y1, double z1, double x2, double y2, double z2) {
		return S2LatLngRect.fromEdge(S2Point.normalize(new S2Point(x1, y1, z1)),
	...
	private static S2LatLngRect rectFromDegrees(double latLo, double lngLo, double latHi, double lngHi) {
	 ...
	 * its result against a.getDistance(c), c being the point rectangle created from
	 * b.
	 */

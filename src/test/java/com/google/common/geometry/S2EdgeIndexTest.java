...
public strictfp class S2EdgeIndexTest extends GeometryTestCase {
	...
	public static class EdgeVectorIndex extends S2EdgeIndex {
		...
		protected S2Point edgeFrom(int index) {
			return this.edges.get(index).getStart();
		}

		@Override
		protected S2Point edgeTo(int index) {
			return this.edges.get(index).getEnd();
		}
	}
	...
	private S2Edge randomEdgeCrossingCap(double maxLengthMeters, S2Cap cap) {
		...
		S2Cap edgeCap = S2Cap.fromAxisAngle(edgeCenter,
				S1Angle.radians(maxLengthMeters / S2LatLng.EARTH_RADIUS_METERS / 2));
	 ...
	 */
	private void generateRandomEarthEdges(double edgeLengthMetersMax, double capSpanMeters, int numEdges,
			List<S2Edge> edges) {
		S2Cap cap = S2Cap.fromAxisAngle(randomPoint(), S1Angle.radians(capSpanMeters / S2LatLng.EARTH_RADIUS_METERS));
		for (int i = 0; i < numEdges; ++i) {
			edges.add(randomEdgeCrossingCap(edgeLengthMetersMax, cap));
		}
	}

	private static void checkAllCrossings(List<S2Edge> allEdges, int minCrossings, int maxChecksCrossingsRatio) {
		...
		for (int in = 0; in < allEdges.size(); ++in) {
			...
			for (int i = 0; i < allEdges.size(); ++i) {
				int crossing = S2EdgeUtil.robustCrossing(e.getStart(), e.getEnd(), allEdges.get(i).getStart(),
						allEdges.get(i).getEnd());
				if (crossing >= 0) {
					StringBuilder sbError = new StringBuilder();
					sbError.append("\n==CHECK_ERROR===================================\n").append("CandidateSet: ")
							.append(sb).append("\nin=").append(in).append(" i=").append(i).append(" robustCrossing=")
		...
		log.info("Pairs/num crossings/check crossing ratio: " + Integer.toString(allEdges.size() * allEdges.size())
				+ "/" + Double.toString(totalCrossings) + "/" + Double.toString(totalIndexChecks / totalCrossings));
	 ...
	 * are candidates.
	 */
	private void tryCrossingsRandomInCap(int numEdges, double edgeLengthMax, double capSpanMeters, int minCrossings,

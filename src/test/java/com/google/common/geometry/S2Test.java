...
public strictfp class S2Test extends GeometryTestCase {
	...
	public void testFaceUVtoXYZ() {
		...
		for (int face = 0; face < 6; ++face) {
			assertEquals(S2Point.crossProd(S2Projections.getUAxis(face), S2Projections.getVAxis(face))
					.dotProd(S2Projections.faceUvToXyz(face, 0, 0)), 1.0);
		}
	...
	public void testUVNorms() {
		...
		for (int face = 0; face < 6; ++face) {
			for (double x = -1; x <= 1; x += 1 / 1024.) {
				assertDoubleNear(
						S2Point.crossProd(S2Projections.faceUvToXyz(face, x, -1), S2Projections.faceUvToXyz(face, x, 1))
								.angle(S2Projections.getUNorm(face, x)),
						0);
				assertDoubleNear(
						S2Point.crossProd(S2Projections.faceUvToXyz(face, -1, x), S2Projections.faceUvToXyz(face, 1, x))
	...
	public void testUVAxes() {
		...
		for (int face = 0; face < 6; ++face) {
			assertEquals(S2Projections.getUAxis(face),
					S2Point.sub(S2Projections.faceUvToXyz(face, 1, 0), S2Projections.faceUvToXyz(face, 0, 0)));
			assertEquals(S2Projections.getVAxis(face),
					S2Point.sub(S2Projections.faceUvToXyz(face, 0, 1), S2Projections.faceUvToXyz(face, 0, 0)));
		}
	}
	...
	public void testAngleArea() {
		...
		double quarterArea1 = S2.area(p000eps2, p000, p090) + S2.area(p000eps2, p090, p180)
		...
		double quarterArea2 = S2.area(p045eps2, p000, p090) + S2.area(p045eps2, p090, p180)
	...
	public void testMetrics() {
		...
		MetricBundle angleSpan = new MetricBundle(S2Projections.MIN_ANGLE_SPAN, S2Projections.MAX_ANGLE_SPAN,
				S2Projections.AVG_ANGLE_SPAN);
		MetricBundle width = new MetricBundle(S2Projections.MIN_WIDTH, S2Projections.MAX_WIDTH,
				S2Projections.AVG_WIDTH);
		MetricBundle edge = new MetricBundle(S2Projections.MIN_EDGE, S2Projections.MAX_EDGE, S2Projections.AVG_EDGE);
		MetricBundle diag = new MetricBundle(S2Projections.MIN_DIAG, S2Projections.MAX_DIAG, S2Projections.AVG_DIAG);
		MetricBundle area = new MetricBundle(S2Projections.MIN_AREA, S2Projections.MAX_AREA, S2Projections.AVG_AREA);
		...
		assertTrue(S2Projections.MAX_EDGE_ASPECT < S2Projections.MAX_EDGE.deriv() / S2Projections.MIN_EDGE.deriv());
		assertTrue(S2Projections.MAX_DIAG_ASPECT >= 1);
		assertTrue(S2Projections.MAX_DIAG_ASPECT < S2Projections.MAX_DIAG.deriv() / S2Projections.MIN_DIAG.deriv());
		...
		assertTrue(S2Projections.MIN_AREA.deriv() >= S2Projections.MIN_WIDTH.deriv() * S2Projections.MIN_EDGE.deriv()
				- 1e-15);
		assertTrue(S2Projections.MAX_AREA.deriv() < S2Projections.MAX_WIDTH.deriv() * S2Projections.MAX_EDGE.deriv()
				+ 1e-15);

...
public strictfp class S2EdgeUtilTest extends GeometryTestCase {
  ...
  private static void assertCrossing(S2Point a,
      S2Point b,
      S2Point c,
      S2Point d,
      int robust,
  ...
  private static void assertCrossings(S2Point a,
      S2Point b,
      S2Point c,
      S2Point d,
      int robust,
  ...
  private static void assertWedge(S2Point a0,
      S2Point ab1,
      S2Point a2,
      S2Point b0,
      S2Point b2,
  ...
  public void testIntersectionTolerance() {
    ...
    for (int i = 0; i < 1000; ++i) {
      ...
      S2Point a = S2Point.normalize(
          S2Point.add(p, S2Point.mul(d1, Math.pow(1e-15 / slope, this.rand.nextDouble()))));
      S2Point b = S2Point.normalize(
          S2Point.sub(p, S2Point.mul(d1, Math.pow(1e-15 / slope, this.rand.nextDouble()))));
      S2Point c = S2Point.normalize(
          S2Point.add(p, S2Point.mul(d2, Math.pow(1e-15 / slope, this.rand.nextDouble()))));
      S2Point d = S2Point.normalize(
          S2Point.sub(p, S2Point.mul(d2, Math.pow(1e-15 / slope, this.rand.nextDouble()))));

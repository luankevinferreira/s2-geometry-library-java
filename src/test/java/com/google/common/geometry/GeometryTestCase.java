...
public strictfp class GeometryTestCase extends TestCase {
	...
	public S2Point randomPoint() {
		return S2Point.normalize(new S2Point(2 * this.rand.nextDouble() - 1, 2 * this.rand.nextDouble() - 1,
	...
	/**
	 * Return a random cell id at the given level or at a randomly chosen level. The
	 * distribution is uniform over the space of cell ids, but only approximately
	 * uniform over the surface of the sphere.
	 */
	public S2CellId getRandomCellId(int level) {
		int face = random(S2CellId.NUM_FACES);
		long pos = this.rand.nextLong() & ((1L << (2 * S2CellId.MAX_LEVEL)) - 1);
	...
	int skewed(int maxLog) {
		final int base = Math.abs(this.rand.nextInt()) % (maxLog + 1);
	 ...
	 * is true, also checks that it does not contain any cells that do not intersect
	 * the given region. ("id" is only used internally.)
	 */
	...
	S2Point samplePoint(S2Cap cap) {
		...
		double h = this.rand.nextDouble() * cap.height();
		double theta = 2 * S2.M_PI * this.rand.nextDouble();

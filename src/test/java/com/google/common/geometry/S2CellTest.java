...
public strictfp class S2CellTest extends GeometryTestCase {
	...
	public void testFaces() {
		...
		for (int face = 0; face < 6; ++face) {
			...
			for (int k = 0; k < 4; ++k) {
				...
				assertDoubleNear(
						S2Point.normalize(S2Point.crossProd(cell.getVertexRaw(k), cell.getVertexRaw((k + 1) & 3)))
	...
	public void testMinMaxAvg(String label, int level, double count, double absError, double minValue, double maxValue,
			double avgValue, S2.Metric minMetric, S2.Metric maxMetric, S2.Metric avgMetric) {
		...
		System.out.printf(
				...
				label, count, tolerance, minError / minValue, minError / tolerance, maxError / maxValue,
				maxError / tolerance, avgError / avgValue, avgError / tolerance);
	...
	public void testSubdivide() {
		...
		for (int i = 0; i <= S2CellId.MAX_LEVEL; ++i) {
			...
			System.out.printf("%5d  %6.3f %6.3f %6.3f %6.3f %6.3f %6.3f %6.3f %6.3f %6.3f\n", i, s.maxArea / s.minArea,
					s.maxEdge / s.minEdge, s.maxEdgeAspect, s.maxDiag / s.minDiag, s.maxDiagAspect, s.minApproxRatio,
		...
		for (int i = 0; i <= S2CellId.MAX_LEVEL; ++i) {
			...
			testMinMaxAvg("area", i, s.count, 1e-15 * s.minWidth, s.minArea, s.maxArea, s.avgArea,
					S2Projections.MIN_AREA, S2Projections.MAX_AREA, S2Projections.AVG_AREA);
			testMinMaxAvg("width", i, s.count, 1e-15, s.minWidth, s.maxWidth, s.avgWidth, S2Projections.MIN_WIDTH,
					S2Projections.MAX_WIDTH, S2Projections.AVG_WIDTH);
			testMinMaxAvg("edge", i, s.count, 1e-15, s.minEdge, s.maxEdge, s.avgEdge, S2Projections.MIN_EDGE,
					S2Projections.MAX_EDGE, S2Projections.AVG_EDGE);
			testMinMaxAvg("diagonal", i, s.count, 1e-15, s.minDiag, s.maxDiag, s.avgDiag, S2Projections.MIN_DIAG,
					S2Projections.MAX_DIAG, S2Projections.AVG_DIAG);
			testMinMaxAvg("angle span", i, s.count, 1e-15, s.minAngleSpan, s.maxAngleSpan, s.avgAngleSpan,
			...
			assertTrue(s.maxEdgeAspect <= S2Projections.MAX_EDGE_ASPECT + 1e-15 * (1 << i));
			assertTrue(s.maxDiagAspect <= S2Projections.MAX_DIAG_ASPECT + 1e-15 * (1 << i));
		}
	}

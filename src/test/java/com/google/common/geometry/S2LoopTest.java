 ...
 *
 * Note that testLoopRelations2() is suppressed because it fails in corner cases
 * due to a problem with S2.robustCCW().
 *
 */
public strictfp class S2LoopTest extends GeometryTestCase {
	...
	public void testBounds() {
		assertTrue(this.candyCane.getRectBound().lng().isFull());
		assertTrue(this.candyCane.getRectBound().latLo().degrees() < -20);
		assertTrue(this.candyCane.getRectBound().latHi().degrees() > 10);
		assertTrue(this.smallNeCw.getRectBound().isFull());
		assertEquals(this.arctic80.getRectBound(),
				new S2LatLngRect(S2LatLng.fromDegrees(80, -180), S2LatLng.fromDegrees(90, 180)));
		assertEquals(this.antarctic80.getRectBound(),
				new S2LatLngRect(S2LatLng.fromDegrees(-90, -180), S2LatLng.fromDegrees(-80, 180)));

		this.arctic80.invert();
		// The highest latitude of each edge is attained at its midpoint.
		S2Point mid = S2Point.mul(S2Point.add(this.arctic80.vertex(0), this.arctic80.vertex(1)), 0.5);
		assertDoubleNear(this.arctic80.getRectBound().latHi().radians(), new S2LatLng(mid).lat().radians());
		this.arctic80.invert();

		assertTrue(this.southHemi.getRectBound().lng().isFull());
		assertEquals(this.southHemi.getRectBound().lat(), new R1Interval(-S2.M_PI_2, 0));
	}

	public void testAreaCentroid() {
		assertDoubleNear(this.northHemi.getArea(), 2 * S2.M_PI);
		assertDoubleNear(this.eastHemi.getArea(), 2 * S2.M_PI);
		...
		for (int i = 0; i < 100; ++i) {
			...
			double height = 2 * this.rand.nextDouble();
			double phi = Math.asin(1 - height);
			double maxDtheta = 2 * Math.acos(Math.tan(Math.abs(phi)) / Math.tan(Math.abs(phi) + kMaxDist));
			maxDtheta = Math.min(S2.M_PI, maxDtheta); // At least 3 vertices.

			List<S2Point> vertices = Lists.newArrayList();
			for (double theta = 0; theta < 2 * S2.M_PI; theta += this.rand.nextDouble() * maxDtheta) {
	...
	public void testContains() {
		...
		for (int i = 0; i < 4; ++i) {
			...
			assertTrue(!this.eastHemi.contains(new S2Point(0, -1, 0)));
			this.northHemi = rotate(this.northHemi);
			this.southHemi = rotate(this.southHemi);
			this.eastHemi = rotate(this.eastHemi);
			this.westHemi = rotate(this.westHemi);
		}
	...
	private static S2CellId advance(S2CellId id, int n) {
	...
	private static void assertRelation(S2Loop a, S2Loop b, int containsOrCrosses, boolean intersects,
			boolean nestable) {
	...
	public void testLoopRelations() {
		assertRelation(this.northHemi, this.northHemi, 1, true, false);
		assertRelation(this.northHemi, this.southHemi, 0, false, false);
		assertRelation(this.northHemi, this.eastHemi, -1, true, false);
		assertRelation(this.northHemi, this.arctic80, 1, true, true);
		assertRelation(this.northHemi, this.antarctic80, 0, false, true);
		assertRelation(this.northHemi, this.candyCane, -1, true, false);

		// We can't compare northHemi3 vs. northHemi or southHemi.
		assertRelation(this.northHemi3, this.northHemi3, 1, true, false);
		assertRelation(this.northHemi3, this.eastHemi, -1, true, false);
		assertRelation(this.northHemi3, this.arctic80, 1, true, true);
		assertRelation(this.northHemi3, this.antarctic80, 0, false, true);
		assertRelation(this.northHemi3, this.candyCane, -1, true, false);

		assertRelation(this.southHemi, this.northHemi, 0, false, false);
		assertRelation(this.southHemi, this.southHemi, 1, true, false);
		assertRelation(this.southHemi, this.farHemi, -1, true, false);
		assertRelation(this.southHemi, this.arctic80, 0, false, true);
		assertRelation(this.southHemi, this.antarctic80, 1, true, true);
		assertRelation(this.southHemi, this.candyCane, -1, true, false);

		assertRelation(this.candyCane, this.northHemi, -1, true, false);
		assertRelation(this.candyCane, this.southHemi, -1, true, false);
		assertRelation(this.candyCane, this.arctic80, 0, false, true);
		assertRelation(this.candyCane, this.antarctic80, 0, false, true);
		assertRelation(this.candyCane, this.candyCane, 1, true, false);

		assertRelation(this.nearHemi, this.westHemi, -1, true, false);

		assertRelation(this.smallNeCw, this.southHemi, 1, true, false);
		assertRelation(this.smallNeCw, this.westHemi, 1, true, false);
		assertRelation(this.smallNeCw, this.northHemi, -2, true, false);
		assertRelation(this.smallNeCw, this.eastHemi, -2, true, false);

		assertRelation(this.loopA, this.loopA, 1, true, false);
		assertRelation(this.loopA, this.loopB, -1, true, false);
		assertRelation(this.loopA, this.aIntersectB, 1, true, false);
		assertRelation(this.loopA, this.aUnionB, 0, true, false);
		assertRelation(this.loopA, this.aMinusB, 1, true, false);
		assertRelation(this.loopA, this.bMinusA, 0, false, false);

		assertRelation(this.loopB, this.loopA, -1, true, false);
		assertRelation(this.loopB, this.loopB, 1, true, false);
		assertRelation(this.loopB, this.aIntersectB, 1, true, false);
		assertRelation(this.loopB, this.aUnionB, 0, true, false);
		assertRelation(this.loopB, this.aMinusB, 0, false, false);
		assertRelation(this.loopB, this.bMinusA, 1, true, false);

		assertRelation(this.aIntersectB, this.loopA, 0, true, false);
		assertRelation(this.aIntersectB, this.loopB, 0, true, false);
		assertRelation(this.aIntersectB, this.aIntersectB, 1, true, false);
		assertRelation(this.aIntersectB, this.aUnionB, 0, true, true);
		assertRelation(this.aIntersectB, this.aMinusB, 0, false, false);
		assertRelation(this.aIntersectB, this.bMinusA, 0, false, false);

		assertRelation(this.aUnionB, this.loopA, 1, true, false);
		assertRelation(this.aUnionB, this.loopB, 1, true, false);
		assertRelation(this.aUnionB, this.aIntersectB, 1, true, true);
		assertRelation(this.aUnionB, this.aUnionB, 1, true, false);
		assertRelation(this.aUnionB, this.aMinusB, 1, true, false);
		assertRelation(this.aUnionB, this.bMinusA, 1, true, false);

		assertRelation(this.aMinusB, this.loopA, 0, true, false);
		assertRelation(this.aMinusB, this.loopB, 0, false, false);
		assertRelation(this.aMinusB, this.aIntersectB, 0, false, false);
		assertRelation(this.aMinusB, this.aUnionB, 0, true, false);
		assertRelation(this.aMinusB, this.aMinusB, 1, true, false);
		assertRelation(this.aMinusB, this.bMinusA, 0, false, true);

		assertRelation(this.bMinusA, this.loopA, 0, false, false);
		assertRelation(this.bMinusA, this.loopB, 0, true, false);
		assertRelation(this.bMinusA, this.aIntersectB, 0, false, false);
		assertRelation(this.bMinusA, this.aUnionB, 0, true, false);
		assertRelation(this.bMinusA, this.aMinusB, 0, false, true);
		assertRelation(this.bMinusA, this.bMinusA, 1, true, false);
	}
	 ...
	 * arbitrary-precision arithmetic to be truly robust. That means it can give the
	 * wrong answers in cases where we are trying to determine edge intersections.
	 * <p>
	 ...
	 * is an attempted workaround in {@link S2Loop#isValid()}, but it does not cover
	 * all cases.
	 */
	public void suppressedTestLoopRelations2() {
		...
		for (int iter = 0; iter < 1000; ++iter) {
			long num = this.rand.nextLong();
			S2CellId begin = new S2CellId(num | 1);
			if (!begin.isValid()) {
				continue;
			}
			begin = begin.parent((int) Math.round(this.rand.nextDouble() * S2CellId.MAX_LEVEL));
			...
			log.info("Checking " + a.numVertices() + " vs. " + b.numVertices() + ", contained = " + contained
	...
	private static void dumpCrossings(S2Loop loop) {
		...
		for (int i = 1; i <= loop.numVertices(); ++i) {
			...
			System.out.printf("Vertex %d: [%.17g, %.17g, %.17g], " + "%d%dR=%d, %d%d%d=%d, R%d%d=%d, inside: %b\n", i,
					loop.vertex(i).x, loop.vertex(i).y, loop.vertex(i).z, i - 1, i, S2.robustCCW(b, o, a), i + 1, i,
					i - 1, S2.robustCCW(c, o, b), i, i + 1, S2.robustCCW(a, o, c), S2.orderedCCW(a, b, c, o));
		}
		for (int i = 0; i < loop.numVertices() + 2; ++i) {
			...
			for (int j = 0; j < loop.numVertices(); ++j) {
				System.out
						.println(" " + S2EdgeUtil.edgeOrVertexCrossing(orig, dest, loop.vertex(j), loop.vertex(j + 1)));
			}
			System.out.println();
		}
		for (int i = 0; i <= 2; i += 2) {
			...
			System.out.printf("%d1R=%d, M1%d=%d, R1M=%d, crosses: %b\n", i, S2.robustCCW(b, o, a), i,
					S2.robustCCW(c, o, b), S2.robustCCW(a, o, c), S2EdgeUtil.edgeOrVertexCrossing(c, o, b, a));
		}
	}
}

...
public strictfp class S2CapTest extends GeometryTestCase {
	...
	public void testBasic() {
		...
		S2Cap tiny = S2Cap.fromAxisAngle(S2Point.normalize(new S2Point(1, 2, 3)), S1Angle.radians(kTinyRad));
		...
		assertTrue(hemi.contains(S2Cap.fromAxisAngle(new S2Point(1, 0, 0), S1Angle.radians(S2.M_PI_4 - EPS))));
		assertTrue(!hemi.contains(S2Cap.fromAxisAngle(new S2Point(1, 0, 0), S1Angle.radians(S2.M_PI_4 + EPS))));
	...
	public void testRectBound() {
		...
		S2LatLngRect rect = S2Cap.fromAxisAngle(getLatLngPoint(-45, 57), S1Angle.degrees(50)).getRectBound();
		...
		rect = S2Cap.fromAxisAngle(S2Point.normalize(new S2Point(1, 0, 1)), S1Angle.radians(S2.M_PI_4)).getRectBound();
		assertDoubleNear(rect.lat().lo(), 0);
		assertDoubleNear(rect.lat().hi(), S2.M_PI_2);
		assertTrue(rect.lng().isFull());

		rect = S2Cap.fromAxisAngle(S2Point.normalize(new S2Point(1, 0, 1)), S1Angle.degrees(45)).getRectBound();
		assertDoubleNear(rect.latLo().degrees(), 0, kDegreeEps);
		assertDoubleNear(rect.latHi().degrees(), 90, kDegreeEps);
		assertTrue(rect.lng().isFull());

		// The eastern hemisphere.
		rect = S2Cap.fromAxisAngle(new S2Point(0, 1, 0), S1Angle.radians(S2.M_PI_2 + 5e-16)).getRectBound();
		assertDoubleNear(rect.latLo().degrees(), -90, kDegreeEps);
		assertDoubleNear(rect.latHi().degrees(), 90, kDegreeEps);
		assertTrue(rect.lng().isFull());
	...
	public void testCells() {
		...
		for (int face = 0; face < 6; ++face) {
			...
			for (S2CellId id = first; id.lessThan(last); id = id.next()) {
				...
				assertEquals(cell.getCapBound().mayIntersect(cornerCell), id.parent().contains(cornerCell.id()));
			}

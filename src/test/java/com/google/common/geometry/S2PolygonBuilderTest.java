...
public strictfp class S2PolygonBuilderTest extends GeometryTestCase {
	...
	private class TestCase {
		...
		public TestCase(int undirectedEdges, int xorEdges, double minMerge, double maxMerge, Chain[] chainsIn,
				String[] loopsOut, int numUnusedEdges) {
	...
	TestCase[] testCases = new TestCase[] {
			...
			new TestCase(0, 1, 0.0, 1.0, // XOR
					new Chain[] { new Chain("0:0, 0:10, 5:15, 10:10, 10:0", true),
							...
							new Chain("14:14, 14:16, 14:18, 14:20", false), new Chain("14:18, 16:20, 18:22", false),
							new Chain("18:12, 16:12, 14:12, 12:12", false),
							new Chain("20:18, 18:16, 16:14, 14:12", false), new Chain("20:14, 18:14, 16:14", false),
			...
			new TestCase(0, 1, 0.0, 4.0, // XOR
					new Chain[] { new Chain("0:0, 0:10, 5:10, 10:10, 10:5, 10:0", true),
							new Chain("0:10, 0:15, 5:15, 5:10", true), new Chain("10:10, 5:10, 5:5, 10:5", true), },
			...
			new TestCase(-1, 0, 0.0, 0.9, // Directed edges required for unique result.
					new Chain[] { new Chain("0:0, 0:2, 0:4, 0:6, 1:5, 2:4, 3:3, 2:2, 1:1", true),
							new Chain("0:2, 1:1, 1:3", true), new Chain("0:4, 1:3, 1:5", true),
							new Chain("1:3, 2:2, 2:4", true), new Chain("0:0, 0:1", false),
							new Chain("1:3, 5:7", false) },
					new String[] { "0:0, 0:2, 1:1", "0:2, 0:4, 1:3", "0:4, 0:6, 1:5", "1:1, 1:3, 2:2", "1:3, 1:5, 2:4",
							"2:2, 2:4, 3:3" },
					2),
			...
			new TestCase(0, -1, 0.0, 4.0, // Don't XOR
					new Chain[] { new Chain("0:0, 0:5, 5:5, 5:0", true), new Chain("0:5, 0:10, 5:10, 5:5", true),
							new Chain("5:0, 5:5, 10:5, 10:0", true), new Chain("5:5, 5:10, 10:10, 10:5", true),
							new Chain("0:10, 0:15, 0:20", false), new Chain("20:0, 15:0, 10:0", false) },
					new String[] { "0:0, 0:5, 5:5, 5:0", "0:5, 0:10, 5:10, 5:5", "5:0, 5:5, 10:5, 10:0",
							"5:5, 5:10, 10:10, 10:5" },
					4),

			// 6: Five nested loops that touch at a point.
			new TestCase(0, 0, 0.0, 0.8,
					new Chain[] { new Chain("0:0, 0:10, 10:10, 10:0", true), new Chain("0:0, 1:9, 9:9, 9:1", true),
							new Chain("0:0, 2:8, 8:8, 8:2", true), new Chain("0:0, 3:7, 7:7, 7:3", true),
			...
			new TestCase(-1, 0, 0.0, 4.0, // Directed edges required for unique result.
					new Chain[] { new Chain("0:-20, -10:0, 0:20, 10:0", true),
							new Chain("0:10, -10:0, 0:-10, 10:0", true), new Chain("0:-10, -5:0, 0:10, 5:0", true),
							new Chain("0:5, -5:0, 0:-5, 5:0", true) },
					new String[] { "0:-20, -10:0, 0:-10, 10:0", "0:-10, -5:0, 0:-5, 5:0", "0:5, -5:0, 0:10, 5:0",
							"0:10, -10:0, 0:20, 10:0" },
					0),

			// 8: Seven diamonds nested within each other touching at one
			// point between each nested pair.
			new TestCase(0, 0, 0.0, 9.0,
					new Chain[] { new Chain("0:-70, -70:0, 0:70, 70:0", true),
							new Chain("0:-70, -60:0, 0:60, 60:0", true), new Chain("0:-50, -60:0, 0:50, 50:0", true),
							new Chain("0:-40, -40:0, 0:50, 40:0", true), new Chain("0:-30, -30:0, 0:30, 40:0", true),
							new Chain("0:-20, -20:0, 0:30, 20:0", true), new Chain("0:-10, -20:0, 0:10, 10:0", true) },
					new String[] { "0:-70, -70:0, 0:70, 70:0", "0:-70, -60:0, 0:60, 60:0", "0:-50, -60:0, 0:50, 50:0",
							"0:-40, -40:0, 0:50, 40:0", "0:-30, -30:0, 0:30, 40:0", "0:-20, -20:0, 0:30, 20:0",
							"0:-10, -20:0, 0:10, 10:0" },
					0),
			...
			new TestCase(0, 1, 1.5, 5.8, // XOR, min_merge > sqrt(2), max_merge < 6.
					new Chain[] { new Chain("-8:-8, -8:0", false), new Chain("-8:1, -8:8", false),
							new Chain("0:-9, -2:0", false), new Chain("-1:1, 1:9", false), new Chain("0:8, 2:2", false),
							new Chain("0:-2, 1:-8", false), new Chain("8:9, 9:1", false), new Chain("9:0, 8:-9", false),
							new Chain("9:-9, 0:-8", false), new Chain("1:-9, -9:-9", false),
							new Chain("8:0, 1:0", false), new Chain("1:2, -8:0", false), new Chain("-8:1, 1:-1", false),
							new Chain("0:1, 8:1", false), new Chain("-9:8, 1:8", false), new Chain("0:9, 8:8", false) },
					new String[] {
							"8.5:8.5, 8.5:0.5, 8.5:-8.5, 0.5:-8.5, " + "-8.5:-8.5, -8.5:0.5, -8.5:8.5, 0.5:8.5" },
					0) };

	private void getVertices(String str, S2Point x, S2Point y, S2Point z, double maxPerturbation,
		...
		for (int i = 0; i < line.numVertices(); ++i) {
			...
			S2Point axis = S2Point
					.normalize(S2Point.add(S2Point.add(S2Point.mul(x, p.x), S2Point.mul(y, p.y)), S2Point.mul(z, p.z)));
			S2Cap cap = S2Cap.fromAxisAngle(axis, S1Angle.radians(maxPerturbation));
			vertices.add(samplePoint(cap));
		}
	}

	private static boolean loopsEqual(S2Loop a, S2Loop b, double maxError) {
	...
	private static boolean findLoop(S2Loop loop, List<S2Loop> candidates, double maxError) {
	...
	boolean findMissingLoops(List<S2Loop> actual, List<S2Loop> expected, double maxError, String label) {
	...
	void addChain(Chain chain, S2Point x, S2Point y, S2Point z, double maxPerturbation, S2PolygonBuilder builder) {
	...
	boolean testBuilder(TestCase test) {
		for (int iter = 0; iter < 200; ++iter) {
			...
			double r = Math.max(0.0, 2 * this.rand.nextDouble() - 1);
			...
			options.setMergeDistance(
					S1Angle.radians(minMerge + 2 * maxPerturbation + r * (maxMerge - minMerge - 4 * maxPerturbation)));
			...
			if (findMissingLoops(loops, expected, maxError, "Actual")
				...
				System.err.print("During iteration " + iter + ", undirected: " + options.getUndirectedEdges()
						+ ", xor: " + options.getXorEdges() + "\n\n");
				return false;
			}
	...
	public void testAssembleLoops() {
		...
		for (int i = 0; i < this.testCases.length; ++i) {
			...
			boolean caseSuccess = testBuilder(this.testCases[i]);

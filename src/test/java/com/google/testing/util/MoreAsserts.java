...
public final class MoreAsserts {
	...
	/**
	 * Asserts that {@code actual} contains precisely the elements {@code expected},
	 * in any order. Both collections may contain duplicates, and this method will
	 * only pass if the quantities are exactly the same.
	 */
	...
	/**
	 * Variant of {@link #assertContentsAnyOrder(String,Iterable,Object...)} using a
	 * generic message.
	 */
	 ...
	 * Asserts that {@code actual} contains precisely the elements {@code expected},
	 * in any order. Both collections may contain duplicates, and this method will
	 * only pass if the quantities are exactly the same. This method uses the
	 * user-provided Comparator object for doing the object comparison, instead of
	 * relying on the contents' implementation of {@link Object#equals(Object)}. It
	 * also takes in the expected set of objects as an Iterable.
	 * <p>
	...
	private static void failNotEqual(String message, Object expected, Object actual) {
		if ((expected != null) && (actual != null) && expected.toString().equals(actual.toString())) {
			failWithMessage(message, "expected:<(" + expected.getClass().getName() + ") " + expected + "> but was:<("

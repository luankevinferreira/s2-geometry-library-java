 ...
 * the entire sphere. They have the following properties: - The id of a cell at
 * level k consists of a 3-bit face number followed by k bit pairs that
 * recursively select one of the four children of each cell. The next bit is
 * always 1, and all other bits are 0. Therefore, the level of a cell is
 * determined by the position of its lowest-numbered bit that is turned on (for
 * a cell at level k, this position is 2 * (MAX_LEVEL - k).) - The id of a
 * parent cell is at the midpoint of the range of ids spanned by its children
 * (or by its descendants at any level).
 *
...
public final strictfp class S2CellId implements Comparable<S2CellId> {
	...
	/**
	 * This is the offset required to wrap around from the beginning of the Hilbert
	 * curve to the end or vice versa; see next_wrap() and prev_wrap().
	 */
	 ...
	 * within that face, and level (range 0..MAX_LEVEL). The given position will be
	 * modified to correspond to the Hilbert curve position at the center of the
	 * returned cell. This is a static function rather than a constructor in order
	 * to give names to the arguments.
	 */
	...
	public S2Point toPointRaw() {
		...
		int delta = isLeaf() ? 1 : (((i.intValue() ^ (((int) this.id) >>> 2)) & 1) != 0) ? 2 : 0;
	...
	/**
	 * Return the child position (0..3) of this cell's ancestor at the given level,
	 * relative to its parent. The argument should be in the range 1..MAX_LEVEL. For
	 * example, child_position(1) returns the position of this cell's level-1
	 * ancestor within its top-level face cell.
	 */
	public int childPosition(int level) {
		return (int) (this.id >>> (2 * (MAX_LEVEL - level) + 1)) & 3;
	}
	...
	public boolean intersects(S2CellId other) {
		// assert (isValid() && other.isValid());
		return other.rangeMin().lessOrEquals(rangeMax()) && other.rangeMax().greaterOrEquals(rangeMin());
	}
	...
	public S2CellId childBegin(int level) {
		...
		return new S2CellId(this.id - lowestOnBit() + lowestOnBitForLevel(level));
	}
	 ...
	 * Should *not* be used for iteration in conjunction with child_begin(),
	 * child_end(), Begin(), or End().
	 */
	 ...
	 * indexing. Cells at lower levels (i.e. larger cells) are encoded into fewer
	 * characters. The maximum token length is 16.
	 *
	 * @param token
	 *            the token to decode
	 ...
	 * regions are at least 100 meters across (level 16 or less), the savings would
	 * be at most 3 bytes (9 bytes hex vs. 6 bytes base-64).
	 *
	 ...
	 * parsing a string representation of a number.
	 */
	...
	public void getEdgeNeighbors(S2CellId neighbors[]) {
		...
		neighbors[0] = fromFaceIJSame(face, i.intValue(), j.intValue() - size, j.intValue() - size >= 0).parent(level);
		...
		neighbors[2] = fromFaceIJSame(face, i.intValue(), j.intValue() + size, j.intValue() + size < MAX_SIZE)
				.parent(level);
		neighbors[3] = fromFaceIJSame(face, i.intValue() - size, j.intValue(), i.intValue() - size >= 0).parent(level);
	}
	...
	public void getVertexNeighbors(int level, List<S2CellId> output) {
		...
		if (isame || jsame) {
			output.add(
					fromFaceIJSame(face, i.intValue() + ioffset, j.intValue() + joffset, isame && jsame).parent(level));
		}
	}
	...
	public void getAllNeighbors(int nbrLevel, List<S2CellId> output) {
		...
		for (int k = -nbrSize;; k += nbrSize) {
			...
			if (k < 0) {
				...
				output.add(fromFaceIJSame(face, i.intValue() + k, j.intValue() - nbrSize, j.intValue() - size >= 0)
						.parent(nbrLevel));
				output.add(fromFaceIJSame(face, i.intValue() + k, j.intValue() + size, j.intValue() + size < MAX_SIZE)
						.parent(nbrLevel));
			}
			// East, West, and Diagonal neighbors.
			output.add(
					fromFaceIJSame(face, i.intValue() - nbrSize, j.intValue() + k, sameFace && i.intValue() - size >= 0)
							.parent(nbrLevel));
	 ...
	 * center of the cell, the returned (i,j) for non-leaf cells will be a leaf cell
	 * adjacent to the cell center. If "orientation" is non-NULL, also return the
	 * Hilbert curve orientation for the current cell.
	 */
	...
	private int getBits1(MutableInteger i, MutableInteger j, int k, int bits) {
		...
		bits += (((int) (this.id >>> (k * 2 * LOOKUP_BITS + 1)) & ((1 << (2 * nbits)) - 1))) << 2;
		/*
		 * System.out.println("id is: " + id_); System.out.println("bits is " + bits);
		 * System.out.println("lookup_ij[bits] is " + lookup_ij[bits]);
		 */
		bits = LOOKUP_IJ[bits];
		i.setValue(i.intValue() + ((bits >> (LOOKUP_BITS + 2)) << (k * LOOKUP_BITS)));
		/*
		 * System.out.println("left is " + ((bits >> 2) & ((1 << kLookupBits) - 1)));
		 * System.out.println("right is " + (k * kLookupBits));
		 * System.out.println("j is: " + j.intValue()); System.out.println("addition is:
		 * " + ((((bits >> 2) & ((1 << kLookupBits) - 1))) << (k * kLookupBits)));
		 */
		j.setValue(j.intValue() + ((((bits >> 2) & ((1 << LOOKUP_BITS) - 1))) << (k * LOOKUP_BITS)));
		bits &= (SWAP_MASK | INVERT_MASK);
		return bits;
	}
	...
	/**
	 * Return the lowest-numbered bit that is on for this cell id, which is equal to
	 * (uint64(1) << (2 * (MAX_LEVEL - level))). So for example, a.lsb() <= b.lsb()
	 * if and only if a.level() >= b.level(), but the first test is more efficient.
	 */
	...
	public boolean lessThan(S2CellId x) {
		return unsignedLongLessThan(this.id, x.id);
	}

	public boolean greaterThan(S2CellId x) {
		return unsignedLongGreaterThan(this.id, x.id);
	}

	public boolean lessOrEquals(S2CellId x) {
		return unsignedLongLessThan(this.id, x.id) || this.id == x.id;
	}

	public boolean greaterOrEquals(S2CellId x) {
		return unsignedLongGreaterThan(this.id, x.id) || this.id == x.id;
	}
	...
	public int compareTo(S2CellId that) {
		return unsignedLongLessThan(this.id, that.id) ? -1 : unsignedLongGreaterThan(this.id, that.id) ? 1 : 0;
	}
}

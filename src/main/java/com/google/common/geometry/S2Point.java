...
public strictfp class S2Point implements Comparable<S2Point> {
	...
	/**
	 * Calcualates hashcode based on stored coordinates. Since we want +0.0 and -0.0
	 * to be treated the same, we ignore the sign of the coordinates.
	 */
	@Override
	public int hashCode() {
		long value = 17;
		value += 37 * value + Double.doubleToLongBits(Math.abs(this.x));
		value += 37 * value + Double.doubleToLongBits(Math.abs(this.y));
		value += 37 * value + Double.doubleToLongBits(Math.abs(this.z));

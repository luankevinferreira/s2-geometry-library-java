...
public final strictfp class S2Cell implements S2Region {
	...
	/**
	 * An S2Cell always corresponds to a particular S2CellId. The other constructors
	...
	/**
	 * Return the inward-facing normal of the great circle passing through the edge
	 * from vertex k to vertex k+1 (mod 4). The normals returned by GetEdgeRaw are
	 * not necessarily unit length.
	 *
	 * If this is not a leaf cell, set children[0..3] to the four children of this
	 * cell (in traversal order) and return true. Otherwise returns false. This
	...
	/**
	 * Return the direction vector corresponding to the center in (s,t)-space of the
	 * given cell. This is the point at which the cell is divided into four
	 ...
	 * S2Projections}). Note that the center of the cell is defined as the point at
	 * which it is recursively subdivided into four children; in general, it is not
	 * at the midpoint of the (u,v) rectangle covered by the cell
	 */
	public R2Vector getCenterUV() {
		...
		// TODO(dbeaumont): Figure out a better naming of the variables here (and
		// elsewhere).
	...
	/**
	 * Return the average area of cells at this level. This is accurate to within a
	 * factor of 1.7 (for S2_QUADRATIC_PROJECTION) and is extremely cheap to
	 * compute.
	 */
	 ...
	 * 3% percent for all cell sizes and accurate to within 0.1% for cells at level
	 * 5 or higher (i.e. 300km square or smaller). It is moderately cheap to
	 * compute.
	 */
	public double approxArea() {
		...
		double flatArea = 0.5 * S2Point
				.crossProd(S2Point.sub(getVertex(2), getVertex(0)), S2Point.sub(getVertex(3), getVertex(1))).norm();
	...
	public S2LatLngRect getRectBound() {
		if (this.level > 0) {
			...
			double u = this.uv[0][0] + this.uv[0][1];
			double v = this.uv[1][0] + this.uv[1][1];
			int i = S2Projections.getUAxis(this.face).z == 0 ? (u < 0 ? 1 : 0) : (u > 0 ? 1 : 0);
			int j = S2Projections.getVAxis(this.face).z == 0 ? (v < 0 ? 1 : 0) : (v > 0 ? 1 : 0);
		...
		case 3:
			return new S2LatLngRect(new R1Interval(-S2.M_PI_4, S2.M_PI_4),
	...
	public boolean contains(S2Point p) {
		...
		return (uvPoint.x() >= this.uv[0][0] && uvPoint.x() <= this.uv[0][1] && uvPoint.y() >= this.uv[1][0]
				&& uvPoint.y() <= this.uv[1][1]);
	}

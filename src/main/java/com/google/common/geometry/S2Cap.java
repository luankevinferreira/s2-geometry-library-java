...
public final strictfp class S2Cap implements S2Region {
  ...
  public S1Angle angle() {
    ...
    return S1Angle.radians(2 * Math.asin(Math.sqrt(0.5 * this.height)));
  }
  ...
  public S2Cap complement() {
    ...
    double cHeight = isFull() ? -1 : 2 - Math.max(this.height, 0.0);
  ...
  public boolean contains(S2Cap other) {
    ...
    return angle().radians() >= this.axis.angle(other.axis)
      + other.angle().radians();
  }
  ...
  public boolean interiorContains(S2Point p) {
    ...
    return isFull() || S2Point.sub(this.axis, p).norm2() < 2 * this.height;
  }
  ...
  public S2Cap addPoint(S2Point p) {
    ...
    if (isEmpty()) {
      ...
      double newHeight = Math.max(this.height, ROUND_UP * 0.5 * dist2);
  ...
  public S2Cap addCap(S2Cap other) {
    if (isEmpty()) {
      ...
      if (angle >= S2.M_PI) {
        ...
        double newHeight = Math.max(this.height, ROUND_UP * 2 * d * d);
        return new S2Cap(this.axis, newHeight);
      }
    }
  }
  ...
  public boolean intersects(S2Cell cell, S2Point[] vertices) {
    ...
    double sin2Angle = this.height * (2 - this.height); // sin^2(capAngle)
    for (int k = 0; k < 4; ++k) {
  ...
  boolean approxEquals(S2Cap other, double maxError) {
    return (this.axis.aequal(other.axis, maxError) && Math.abs(this.height - other.height) <= maxError)

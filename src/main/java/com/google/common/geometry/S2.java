...
public final strictfp class S2 {
	...
	/**
	 * Returns an XOR bit mask indicating how the orientation of a child subcell is
	 * related to the orientation of its parent cell. The returned value can be
	 * XOR'd with the parent cell's orientation to give the orientation of the child
	 * cell.
	 *
	 * @param position
	 *            the position of the subcell in the Hilbert traversal, in the range
	 *            [0,3].
	...
	/**
	 * Return the IJ-index of the subcell at the given position in the Hilbert curve
	 * traversal with the given orientation. This is the inverse of
	 * {@link #ijToPos}.
	 *
	 * @param orientation
	 *            the subcell orientation, in the range [0,3].
	 * @param position
	 *            the position of the subcell in the Hilbert traversal, in the range
	 *            [0,3].
	...
	public static int posToIJ(int orientation, int position) {
		if (!(0 <= orientation && orientation < 4)) {
			throw new IllegalArgumentException(
					String.format("Orientation should be in range [0,3] but got %d", orientation));
		}
		if (!(0 <= position && position < 4)) {
			throw new IllegalArgumentException(String.format("Position should be in range [0,3] but got %d", position));
		}
	...
	public static final int ijToPos(int orientation, int ijIndex) {
		...
		if (!(0 <= ijIndex && ijIndex < 4)) {
			throw new IllegalArgumentException(
					String.format("Index of ij should be in range [0,3] but got %d", ijIndex));
		}
	...
	public static class Metric {
		...
		/**
		 * The "deriv" value of a metric is a derivative, and must be multiplied by a
		 * length or area in (s,t)-space to get a useful value.
		 */
		public double deriv() {
			return this.deriv;
		}
		...
		/**
		 * Return the level at which the metric has approximately the given value. For
		 * example, S2::kAvgEdge.GetClosestLevel(0.1) returns the level at which the
		 * average cell edge length is approximately 0.1. The return value is always a
		 * valid level.
		 */
		 ...
		 * S2CellId::kMaxLevel if there is no such level. For example,
		 * S2::kMaxDiag.GetMinLevel(0.1) returns the minimum level such that all cell
		 * diagonal lengths are 0.1 or smaller. The return value is always a valid
		 * level.
		 */
		public int getMinLevel(double value) {
			...
			int exponent = exp(value / ((1 << this.dim) * this.deriv));
			int level = Math.max(0, Math.min(S2CellId.MAX_LEVEL, -((exponent - 1) >> (this.dim - 1))));
		...
		/**
		 * Return the maximum level such that the metric is at least the given value, or
		 * zero if there is no such level. For example, S2.kMinWidth.GetMaxLevel(0.1)
		 * returns the maximum level such that all cells have a minimum width of 0.1 or
		 * larger. The return value is always a valid level.
		 */
		public int getMaxLevel(double value) {
			...
			int exponent = exp((1 << this.dim) * this.deriv / value);
			int level = Math.max(0, Math.min(S2CellId.MAX_LEVEL, ((exponent - 1) >> (this.dim - 1))));
	 ...
	 * better job of ensuring orthogonality when "a" is nearly parallel to "b", and
	 * it returns a non-zero result even when a == b or a == -b.
	 *
	 * It satisfies the following properties (RCP == RobustCrossProd):
	 *
	 * (1) RCP(a,b) != 0 for all a, b (2) RCP(b,a) == -RCP(a,b) unless a == b or a
	 * == -b (3) RCP(-a,b) == -RCP(a,b) unless a == b or a == -b (4) RCP(a,-b) ==
	...
	/**
	 * Return the area of triangle ABC. The method used is about twice as expensive
	 * as Girard's formula, but it is numerically stable for both large and very
	 * small triangles. The points do not need to be normalized. The area is always
	 * positive.
	 *
	...
	/**
	 * Like Area(), but returns a positive value for counterclockwise triangles and
	 ...
	 * triangle, i.e. the intersection of the three medians. However, note that for
	 * large spherical triangles the surface centroid may be nowhere near the
	...
	public static S2Point trueCentroid(S2Point a, S2Point b, S2Point c) {
		...
		return new S2Point(0.5 * S2Point.crossProd(y, z).dotProd(r), 0.5 * S2Point.crossProd(z, x).dotProd(r),
				0.5 * S2Point.crossProd(x, y).dotProd(r));
	}

	/**
	 * Return true if the points A, B, C are strictly counterclockwise. Return false
	 * if the points are clockwise or colinear (i.e. if they are all contained on
	 * some great circle).
	 *
	 ...
	 *
	 * <p>
	 * Like SimpleCCW(), but returns +1 if the points are counterclockwise and -1 if
	 * the points are clockwise. It satisfies the following conditions:
	 *
	 ...
	 * In other words:
	 *
	 * (1) The result is zero if and only if two points are the same. (2) Rotating
	 * the order of the arguments does not affect the result. (3) Exchanging any two
	 * arguments inverts the result.
	 *
	 ...
	 * properties hold even when the three points are coplanar, and to deal with the
	...
	public static int robustCCW(S2Point a, S2Point b, S2Point c, S2Point aCrossB) {
		...
		// assert ((Math.abs(det) < kMinAbsValue) || (Math.abs(det) > 1000 *
		// kMinAbsValue)
	...
	/**
	 * A relatively expensive calculation invoked by RobustCCW() if the sign of the
	 ...
	 * sweeping CCW around the point O. You can think of this as testing whether A
	 * <= B <= C with respect to a continuous CCW ordering around O.
	 *
	 * Properties:
	 * <ol>
	 ...
	 * always in the range [0, Pi]. The points do not need to be normalized. Ensures
	 * that Angle(a,b,c) == Angle(c,b,a) for all a,b,c.
	 *
	 ...
	 * imagine an ant walking from A to B to C, this is the angle that the ant turns
	 * at vertex B (positive = left, negative = right). Ensures that
	...
	/**
	 * Return true if two points are within the given distance of each other (mainly
	 * useful for testing).
	 */

...
public strictfp class S2EdgeUtil {
	/**
	 * IEEE floating-point operations have a maximum error of 0.5 ULPS (units in the
	 * last place). For double-precision numbers, this works out to 2**-53 (about
	 * 1.11e-16) times the magnitude of the result. It is possible to analyze the
	 * calculation done by getIntersection() and work out the worst-case rounding
	 * error. I have done a rough version of this, and my estimate is that the worst
	 * case distance from the intersection point X to the great circle through (a0,
	 * a1) is about 12 ULPS, or about 1.3e-15. This needs to be increased by a
	 * factor of (1/0.866) to account for the edgeSpliceFraction() in
	 * S2PolygonBuilder. Note that the maximum error measured by the unittest in
	 * 1,000,000 trials is less than 3e-16.
	 */
	public static final S1Angle DEFAULT_INTERSECTION_TOLERANCE = S1Angle.radians(1.5e-15);

	/**
	 * This class allows a vertex chain v0, v1, v2, ... to be efficiently tested for
	 * intersection with a given fixed edge AB.
	 */
	public static class EdgeCrosser {
		 ...
		 * All parameters must point to fixed storage that persists for the lifetime of
		 * the EdgeCrosser object.
		 */
		...
		/**
		 * This method is equivalent to calling the S2EdgeUtil.robustCrossing() function
		 * (defined below) on the edges AB and CD. It returns +1 if there is a crossing,
		 * -1 if there is no crossing, and 0 if two points from different edges are the
		 * same. Returns 0 or -1 if either edge is degenerate. As a side effect, it
		 ...
		 * defined below. It is similar to robustCrossing, but handles cases where two
		 * vertices are identical in a way that makes it easy to implement
	...
	/**
	 * This class computes a bounding rectangle that contains all edges defined by a
	 * vertex chain v0, v1, v2, ... All vertices must be unit length. Note that the
	 * bounding rectangle of an edge can be larger than the bounding rectangle of
	 * its endpoints, e.g. consider an edge that passes through the north pole.
	 */
	public static class RectBounder {
		...
		/**
		 * Return the bounding rectangle of the edge chain that connects the vertices
		 * defined so far.
		 */
	 ...
	 * bounding box. It can be used as an efficient rejection test when attempting
	 * to find edges that intersect a given region. It accepts a vertex chain v0,
	 * v1, v2, ... and returns a boolean value indicating whether each edge
	 * intersects the specified bounding box.
	 *
	 * We use XYZ intervals instead of something like longitude intervals because it
	 * is cheap to collect from S2Point lists and any slicing strategy should give
	 * essentially equivalent results. See S2Loop for an example of use.
	 */
	public static class XYZPruner {
		 ...
		 * through the pruner bounding box, otherwise returns false. So the method
		 * returns false if we are certain there is no intersection, but it may return
		 * true when there turns out to be no intersection.
		 */
		public boolean intersects(S2Point v1) {
			boolean result = true;

			if ((v1.x < this.xmin && this.lastVertex.x < this.xmin)
					|| (v1.x > this.xmax && this.lastVertex.x > this.xmax)) {
				result = false;
			} else if ((v1.y < this.ymin && this.lastVertex.y < this.ymin)
					|| (v1.y > this.ymax && this.lastVertex.y > this.ymax)) {
				result = false;
			} else if ((v1.z < this.zmin && this.lastVertex.z < this.zmin)
					|| (v1.z > this.zmax && this.lastVertex.z > this.zmax)) {
				result = false;
			}
	 ...
	 *
	 * This class is not currently used as the XYZPruner is preferred for S2Loop,
	 * but this should be usable in similar circumstances. Be wary of the cost of
	 * atan2() in conversions from S2Point to longitude!
	 */
	public static class LongitudePruner {
		...
		/**
		 * 'interval' is the longitude interval to be tested against, and 'v0' is the
		 * first vertex of edge chain.
		 */
		...
		/**
		 * Returns true if the edge (v0, v1) intersects the given longitude interval,
		 * and then saves 'v1' to be used as the next 'v0'.
		 */
		public boolean intersects(S2Point v1) {
	 ...
	 * cases (such as a0 == b2) are handled as expected. The parameter "ab1" denotes
	 * the common vertex a1 == b1.
	 */
	...
	public static class WedgeContains implements WedgeRelation {
		/**
		 * Given two edge chains (see WedgeRelation above), this function returns +1 if
		 * the region to the left of A contains the region to the left of B, and 0
		 * otherwise.
		 */
		@Override
	...
	public static class WedgeIntersects implements WedgeRelation {
		/**
		 * Given two edge chains (see WedgeRelation above), this function returns -1 if
		 * the region to the left of A intersects the region to the left of B, and 0
		 * otherwise. Note that regions are defined such that points along a boundary
		 * are contained by one side or the other, not both. So for example, if A,B,C
		 * are distinct points ordered CCW around a vertex O, then the wedges BOA, AOC,
		 * and COB do not intersect.
		 */
		@Override
		public int test(S2Point a0, S2Point ab1, S2Point a2, S2Point b0, S2Point b2) {
	...
	public static class WedgeContainsOrIntersects implements WedgeRelation {
		/**
		 * Given two edge chains (see WedgeRelation above), this function returns +1 if
		 * A contains B, 0 if A and B are disjoint, and -1 if A intersects but does not
		 * contain B.
		 */
		@Override
		public int test(S2Point a0, S2Point ab1, S2Point a2, S2Point b0, S2Point b2) {
	...
	public static class WedgeContainsOrCrosses implements WedgeRelation {
		/**
		 * Given two edge chains (see WedgeRelation above), this function returns +1 if
		 * A contains B, 0 if B contains A or the two wedges do not intersect, and -1 if
		 * the edge chains A and B cross each other (i.e. if A intersects both the
		 * interior and exterior of the region to the left of B). In degenerate cases
		 * where more than one of these conditions is satisfied, the maximum possible
		 * result is returned. For example, if A == B then the result is +1.
		 */
		@Override
		public int test(S2Point a0, S2Point ab1, S2Point a2, S2Point b0, S2Point b2) {
	 ...
	 * rules of S2.robustCCW). It returns +1 if there is a crossing, -1 if there is
	 * no crossing, and 0 if any two vertices from different edges are the same.
	 * Returns 0 or -1 if either edge is degenerate. Properties of robustCrossing:
	 *
	 ...
	 *
	 * Note that if you want to check an edge against a *chain* of other edges, it
	 * is much more efficient to use an EdgeCrosser (above).
	 */
	 ...
	 * implemented by counting the number of edge crossings. The basic rule is that
	 * a "crossing" occurs if AB is encountered after CD during a CCW sweep around
	 * the shared vertex starting from a fixed reference point.
	 *
	 * Note that according to this rule, if AB crosses CD then in general CD does
	 * not cross AB. However, this leads to the correct result when counting polygon
	 * edge crossings. For example, suppose that A,B,C are three consecutive
	 * vertices of a CCW polygon. If we now consider the edge crossings of a segment
	 * BP as P sweeps around B, the crossing number changes parity exactly when BP
	 * crosses BA or BC.
	 *
	...
	/**
	 * A convenience function that calls robustCrossing() to handle cases where all
	 * four vertices are distinct, and VertexCrossing() to handle cases where two or
	 * more vertices are the same. This defines a crossing function such that
	 * point-in-polygon containment tests can be implemented by simply counting edge
	 * crossings.
	 */
	...
	/**
	 * Given a point X and an edge AB, return the distance ratio AX / (AX + BX). If
	 * X happens to be on the line segment AB, this is the fraction "t" such that X
	 * == Interpolate(A, B, t). Requires that A and B are distinct.
	 */
	...
	/**
	 * Return the minimum distance from X to any point on the edge AB. The result is
	 * very accurate for small distances but may have some numerical error if the
	 * distance is large (approximately Pi/2 or greater). The case A == B is handled
	 * correctly. Note: x, a and b must be of unit length. Throws
	...
	/**
	 * A slightly more efficient version of getDistance() where the cross product of
	 * the two endpoints has been precomputed. The cross product does not need to be
	 * normalized, but should be computed using S2.robustCrossProd() for the most
	 * accurate results.
	 */
	public static S1Angle getDistance(S2Point x, S2Point a, S2Point b, S2Point aCrossB) {
		if (!S2.isUnitLength(x) || !S2.isUnitLength(a) || !S2.isUnitLength(b)) {
			throw new IllegalArgumentException();
		}

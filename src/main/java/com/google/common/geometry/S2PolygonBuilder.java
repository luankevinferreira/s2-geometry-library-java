...
public strictfp class S2PolygonBuilder {
  ...
  public void addEdge(S2Point v0, S2Point v1) {
    ...
    if (this.options.getXorEdges()) {
      Multiset<S2Point> candidates = this.edges.get(v1);
    ...
    if (this.edges.get(v0) == null) {
      this.edges.put(v0, HashMultiset.<S2Point>create());
    }

    this.edges.get(v0).add(v1);
    if (this.options.getUndirectedEdges()) {
      if (this.edges.get(v1) == null) {
        this.edges.put(v1, HashMultiset.<S2Point>create());
      }
  ...
  public boolean assembleLoops(List<S2Loop> loops, List<S2Edge> unusedEdges) {
    if (this.options.getMergeDistance().radians() > 0) {
      mergeVertices();
    }
    ...
    while (!this.edges.isEmpty()) {
      Map.Entry<S2Point, Multiset<S2Point>> edge = this.edges.entrySet().iterator().next();
    ...
    for (Map.Entry<S2Point, Multiset<S2Point>> edge : this.edges.entrySet()) {
      ...
      for (S2Point v1 : vset) {
        if (mergeMap.get(v0) != null || mergeMap.get(v1) != null) {
          ...
          if (!this.options.getUndirectedEdges() || v0.lessThan(v1)) {
    ...
    public void query(S2Point center, List<S2Point> output) {
      ...
      for (S2CellId id : neighbors) {
        ...
        for (MarkedS2Point mp : get(id)) {
          ...
          if (center.angle(p) <= this.searchRadius) {
            output.add(p);
            mp.mark();
          }
        }

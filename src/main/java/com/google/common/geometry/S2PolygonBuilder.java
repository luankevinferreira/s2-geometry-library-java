 ...
 *
 * 2. Constructing polygons from input data that does not follow S2 conventions,
 * i.e. where loops may have repeated vertices, or distinct loops may share
 * edges, or shells and holes have opposite or unspecified orientations.
 *
...
public strictfp class S2PolygonBuilder {
	...
	/**
	 * The current set of edges, grouped by origin. The set of destination vertices
	...
	public enum Options {
		...
		/**
		 * These are the options that should be used for assembling well-behaved input
		 * data into polygons. All edges should be directed such that "shells" and
		 * "holes" have opposite orientations (typically CCW shells and clockwise
		 * holes), unless it is known that shells and holes do not share any edges.
		 */
		DIRECTED_XOR(false, true),

		/**
		 * These are the options that should be used for assembling polygons that do not
		 * follow the conventions above, e.g. where edge directions may vary within a
		 * single loop, or shells and holes are not oppositely oriented.
		 */
		UNDIRECTED_XOR(true, true),

		/**
		 * These are the options that should be used for assembling edges where the
		 * desired output is a collection of loops rather than a polygon, and edges may
		 * occur more than once. Edges are treated as undirected and are not XORed
		 * together, in particular, adding edge A->B also adds B->A.
		 */
		UNDIRECTED_UNION(true, false),

		/**
		 * Finally, select this option when the desired output is a collection of loops
		 * rather than a polygon, but your input edges are directed and you do not want
		 * reverse edges to be added implicitly as above.
		 */
		...
		/**
		 * If "undirected_edges" is false, then the input is assumed to consist of edges
		 * that can be assembled into oriented loops without reversing any of the edges.
		 * Otherwise, "undirected_edges" should be set to true.
		 */
		...
		/**
		 * If "xor_edges" is true, then any duplicate edge pairs are removed. This is
		 * useful for computing the union of a collection of polygons whose interiors
		 * are disjoint but whose boundaries may share some common edges (e.g. computing
		 * the union of South Africa, Lesotho, and Swaziland).
		 *
		 * Note that for directed edges, a "duplicate edge pair" consists of an edge and
		 * its corresponding reverse edge. This means that either (a) "shells" and
		 * "holes" must have opposite orientations, or (b) shells and holes do not share
		 * edges. Otherwise undirected_edges() should be specified.
		 *
		 * There are only two reasons to turn off xor_edges():
		 *
		 * (1) assemblePolygon() will be called, and you want to assert that there are
		 * no duplicate edge pairs in the input.
		 *
		...
		/**
		 * If true, isValid() is called on all loops and polygons before constructing
		 * them. If any loop is invalid (e.g. self-intersecting), it is rejected and
		 * returned as a set of "unused edges". Any remaining valid loops are kept. If
		 * the entire polygon is invalid (e.g. two loops intersect), then all loops are
		 * rejected and returned as unused edges.
		 */
		...
		/**
		 * If set to a positive value, all vertices that are separated by at most this
		 * distance will be merged together. In addition, vertices that are closer than
		 * this distance to a non-incident edge will be spliced into it (TODO).
		 *
	 ...
	 * not allowed to cross each other. Also note that as a convenience, edges where
	 * v0 == v1 are ignored.
	 */
	public void addEdge(S2Point v0, S2Point v1) {
		...
		if (edges.get(v0) == null) {
			edges.put(v0, new ArrayList<S2Point>());
		}

		edges.get(v0).add(v1);
		if (options.getUndirectedEdges()) {
			if (edges.get(v1) == null) {
				edges.put(v1, new ArrayList<S2Point>());
			}
			edges.get(v1).add(v0);
		}
	}
	...
	/**
	 * Add all edges in the given loop. If the sign() of the loop is negative (i.e.
	 * this loop represents a hole), the reverse edges are added instead. This
	 * implies that "shells" are CCW and "holes" are CW, as required for the
	...
	/**
	 * Add all loops in the given polygon. Shells and holes are added with opposite
	 * orientations as described for AddLoop(). This method does not take ownership
	 * of the polygon.
	 */
	 ...
	 * preferred. Returns true if all edges were assembled. If "unused_edges" is not
	 * NULL, it is initialized to the set of edges that could not be assembled into
	 * loops.
	 *
	 * Note that if xor_edges() is false and duplicate edge pairs may be present,
	 * then undirected_edges() should be specified unless all loops can be assembled
	 * in a counter-clockwise direction. Otherwise this method may not be able to
	 * assemble all loops due to its preference for CCW loops.
	 *
	 ...
	 *
	 * For this method to succeed, there should be no duplicate edges in the input.
	 * If this is not known to be true, then the "xor_edges" option should be set
	 * (which is true by default).
	 *
	 * Note that S2Polygons cannot represent arbitrary regions on the sphere,
	 * because of the limitation that no loop encloses more than half of the sphere.
	 * For example, an S2Polygon cannot represent a 100km wide band around the
	 * equator. In such cases, this method will return the *complement* of the
	 * expected region. So for example if all the world's coastlines were assembled,
	 * the output S2Polygon would represent the land area (irrespective of the input
	 * edge or loop orientations).
	 */
	 ...
	 * given search radius of an arbitrary query location. It is essentially just a
	 * hash map from cell ids at a given fixed level to the set of points contained
	 * by that cell id.
	 *
	 * This class is not suitable for general use because it only supports
	 * fixed-radius queries and has various special-purpose operations to avoid the
	...
	private class PointIndex extends HashMap<S2CellId, Collection<MarkedS2Point>> {
		...
		public PointIndex(double searchRadius) {
			...
			this.level = Math.min(S2Projections.MIN_WIDTH.getMaxLevel(2 * searchRadius), S2CellId.MAX_LEVEL - 1);
		}
		...
		/**
		 * Return the set the unmarked points whose distance to "center" is less than
		 * search_radius_, and mark these points. By construction, these points will be
		 * contained by one of the vertex neighbors of "center".
		 */
		...
		public Collection<Map.Entry<S2CellId, MarkedS2Point>> entries() {
			...
			for (Map.Entry<S2CellId, Collection<MarkedS2Point>> entry : entrySet()) {
